...
The standing instructions the skills assume. This is the candidate replacement for
the terse `DEFAULT_SYSTEM_PROMPT` in `bosc.agent.client`; adopting it in-app is
tracked as a follow-up (see [`README.md`](README.md)). The skills under
`.claude/skills/` specialize this posture; [`ENRICHMENT.md`](ENRICHMENT.md) binds
it to Project BOSC's facts and formats.
