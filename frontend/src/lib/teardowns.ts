/**
 * Curated Record Teardowns for the guided walk. These are the editorial framings
 * of two anchor records — the Tetra Tech Opinion of Probable Cost and the
 * Roadwork Development Agreement — ported from the design handoff. The figures
 * are corpus-grounded; the `connect` chips and `verify`/`method` links are
 * library doors (they point at the reference-library indexes the spine pays off
 * into). When the chapter pages land, a teardown becomes "pick a record, write
 * three sentences" over these same shapes.
 */
...
export const DEED_TEARDOWN: TeardownRecord = {
  ...
  extraction: [
    { label: "Instrument", value: "Limited Warranty Deed" },
    { label: "Recorded", value: "2025-08-13" },
...
export const OPC_TEARDOWN: TeardownRecord = {
  ...
  source: {
    ...
    badge: "SCAN",
    note: "Source binary not materialized in this deploy — provenance is committed; the scan is available on request.",
  },
  extraction: [
    { label: "Program total", value: "$14,223,081" },
    { label: "Sub-estimates", value: "6 corridors" },
    { label: "Contingency", value: "25%" },
...
export const AIR_TEARDOWN: TeardownRecord = {
  ...
  extraction: [
    ...
    { label: "NOx cap", value: "235.62 tpy" },
    { label: "CO cap", value: "96.06 tpy" },
