// Watermark — a record screen, data-driven. A variant switcher (Cost · Air permit · NPDES)
// drives one teardown: the source, what we read, what it reveals + figures, how to check,
// gaps in the record, and where it connects. Shows the evidence grammar across record types.
function RecordScreen({ onBack, onOpenProfile }) {
  ...
  const VARIANTS = {
    cost: {
      tab: "Cost estimate",
      eyebrow: "Record · cost estimate",
      title: "Opinion of Probable Cost",
      meta: "Tetra Tech · BOSC Roadwork · filed 2025-07-11",
      source: { file: "PRR-01-bundle.pdf", badge: "SCAN", pages: "pp. 317–328", collection: "Public Records Request 01" },
      read: [
        ["Program total", "$14,223,081", "verified"],
        ["Sub-estimates", "6 corridors", "verified"],
        ["Contingency", "25%", "verified"],
        ["Largest — Cole St", "$3,899,800", "verified"],
        ["Drainage line", "$1,068,530 · 7.5%", "inference"],
        ["Design-storm basis", "~ not cited", "open"],
    ...
    air: {
      tab: "Air permit",
      eyebrow: "Record · air permit-to-install",
      title: "Air Permit-to-Install P0138965",
      meta: "Ohio EPA · 115 emissions units · finalized 2026-05-28",
      source: { file: "OEPA_PTI_P0138965.pdf", badge: "CBI", pages: "§ B.1 emissions table", collection: "Ohio EPA · public notice" },
      read: [
        ["Emissions units", "115", "verified"],
        ["Matched groups", "3 × identical", "verified"],
        ["Per-engine power", "▮▮▮ CBI", "gap"],
        ["Total backup power", "~ 313 MW", "inference"],
        ["Annual run hours", "▮▮▮ CBI", "gap"],
        ["NOₓ basis", "draft only", "open"],
      ],
      figures: [
        { label: "Backup generation", value: "~313", unit: "MW (modeled)", evidence: "inference", basis: "modeled", sub: "115 units × draft per-engine kW", warn: true },
      ],
      reveals: ["The final permit lists ", { b: "115 generators" }, " in three matched groups, but knocks out per-engine power and run hours as confidential business information. The ~313 MW total survives only because the ", { b: "draft" }, " carried the figure before it was redacted — the record reassembles what the final filing hid."],
    ...
    water: {
      tab: "NPDES · water",
      eyebrow: "Record · NPDES discharge permit",
      title: "Cooling-water screen — Ottawa River",
      meta: "NPDES + USGS 04187100 · the 7Q10 low-flow test",
      source: { file: "NPDES_OH_cooling.pdf", badge: "SCAN", pages: "outfall 001 · cooling", collection: "Ohio EPA · NPDES file" },
      read: [
        ["Consumptive draw", "4.85 cfs", "inference"],
        ["Design low flow (7Q10)", "0.2 cfs", "verified"],
        ["Intake of record", "~ not named", "open"],
        ["Dilution at 7Q10", "below 1:1", "inference"],
  ...
  const toc = [["①", "The source", true], ["②", "What we read", false], ["③", "What it reveals", false], ["④", "Where it connects", false]];
    ...
    <div style={{ color: "var(--ink)", display: "grid", gridTemplateColumns: "210px 1fr", gap: 0 }}>
      ...
      <aside style={{ borderRight: "1px solid var(--line-hair)", padding: "26px 18px", background: "var(--bone-raised)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {toc.map(([mark, label, on]) => (
            <div key={label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", background: on ? "var(--forest-tint)" : "transparent", cursor: "pointer" }}>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 600, color: on ? "var(--forest)" : "var(--ink-ghost)", width: 14, textAlign: "center" }}>{mark}</span>
              <span style={{ fontSize: 12.5, color: on ? "var(--ink)" : "var(--ink-muted)", fontWeight: on ? 700 : 400 }}>{label}</span>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--line-hair)" }}>
          <Eyebrow tone="faint" style={{ marginBottom: 9 }}>Record type</Eyebrow>
      ...
      <div style={{ padding: "28px 32px 36px" }}>
        ...
        {/* the source + what we read */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginTop: 22 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}><AnnotationPin n="①" size={20} /><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "1px", textTransform: "uppercase", color: "var(--ink-faint)" }}>The source</span></div>
            <SourceCard file={v.source.file} badge={v.source.badge} pages={v.source.pages} collection={v.source.collection} style={{ maxWidth: "100%" }} />
          </div>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}><AnnotationPin n="②" size={20} /><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "1px", textTransform: "uppercase", color: "var(--ink-faint)" }}>What we read from it</span></div>
        ...
        {/* figures (variant-specific) */}
        {v.figures.length ? (
          <div style={{ display: "grid", gridTemplateColumns: v.figures.length > 1 ? "1fr 1fr" : "1fr", gap: 14, marginTop: 18 }}>
            {v.figures.map((f, i) => <FigureStat key={i} {...f} />)}
          </div>
        ) : null}

        {/* what it reveals + how to check */}
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 18, marginTop: 22 }}>
          <div style={{ border: "1px solid var(--line-hair)", background: "var(--surface-card)", padding: "16px 18px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}><AnnotationPin n="③" size={20} /><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "1px", textTransform: "uppercase", color: "var(--ink-faint)" }}>What it reveals</span></div>
          ...
          <div style={{ border: "1px solid var(--line-hair)", background: "var(--bone-sunk)", padding: "16px 18px" }}>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "1px", textTransform: "uppercase", color: "var(--ink-faint)", marginBottom: 12 }}>How to check it</div>
        ...
        {/* gaps in the record (variant-specific) */}
        {v.gaps ? (
          <div style={{ marginTop: 18, border: "1px solid var(--ev-gap-border)", background: "var(--ev-gap-bg)", padding: "13px 16px" }}>
        ...
        <div style={{ marginTop: 22 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 11 }}><AnnotationPin n="④" size={20} /><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "1px", textTransform: "uppercase", color: "var(--ink-faint)" }}>Where it connects</span></div>
