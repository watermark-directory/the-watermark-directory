...
| [`bundle/`](bundle/README.md) | The **typed content bundle** the new frontend reads — versioned JSON feeds + JSON Schemas + a manifest, written by `bosc export`. The contract (README, `schemas/`, `manifest.example.json`) is committed; the feed data is regenerable and git-ignored. See its [README](bundle/README.md). |
