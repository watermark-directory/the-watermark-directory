...
- [~] **Data-center activity** — self-research first pass run (#247); **the weakest leg in the corpus**. **Highland55 primary instruments now in corpus** (PR #803: 10 OEPA/Corps docs in `permits/highland55/` — Section 401 WQC applications + Army Corps JDs for "Urbana Commerce Park" / "Urbana Vance Brands Project" by Highland Realty Development / Urbana Owner I LLC, US Rt 68; `[open]` whether Vance Brands is a data-center end use — see #447). **Honda "Champaign-area" candidate downgraded** [2026-06-28]: `corridor_context: true` in `cloud-consumer-candidates.yaml`; no primary instrument pins the investment to Urbana/Champaign County. See self-research summary below.
