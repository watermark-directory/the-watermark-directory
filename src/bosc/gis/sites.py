...
profile, not a layer of ``gis-findings.geojson``. ``bosc.gis`` is a **consumer** of
``bosc.poi`` — this reads ``tracked_pois()`` and exposes each as a ``TrackingSite`` whose
...
from bosc.poi.store import load_poi, tracked_pois
