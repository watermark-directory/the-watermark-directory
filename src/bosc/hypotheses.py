...
This is **not** :class:`bosc.hydrology.hypothesis.Hypothesis`, which tags water-balance
...
The peer modules are :mod:`bosc.sites` (the site axis) and :mod:`bosc.research` (the
agent that fills cells). Mirrors ``bosc.sites`` conventions: a frozen registry, a
...
class Citation(BaseModel):
    ...
    Field-compatible with :class:`bosc.site.feeds.Citation` so a cell exports to the
...
class HypothesisAssessment(BaseModel):
    ...
    may name a *tracking* candidate not yet in :data:`bosc.sites.SITES` (the frontend lists
...
# --- the registry (ported from frontend/src/lib/directory.ts LENSES) -----------------------
