...
:class:`bosc.site.feeds.Citation`, :class:`bosc.hypotheses.Citation`, and
:class:`bosc.hydrology.model.ProvenancedValue`. Each used to hand-mirror the same
``SourceKind``/``Confidence`` literals and the same ``verified`` predicate; those live here
once so the discipline can't drift. Kept dependency-free (no ``bosc.site`` / ``bosc.hydrology``
