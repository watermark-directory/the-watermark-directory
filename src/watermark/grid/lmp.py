...
each :class:`watermark.sites.SiteProfile` (``lmp_usd_mwh``). Zonal because a site's pricing
...
pydantic`` over :func:`watermark.connectors.cached_get`, reusing the **econ** cache / offline
/ fixtures (the other grid connectors do). The PJM **subscription key** is read from
``settings.pjm_api_key`` (``WATERMARK_PJM_API_KEY``) and sent only as the live request's
    ...
    *,
    ...
    def fetch() -> Any:
        ...
        if not settings.pjm_api_key:
            raise PjmLmpError(
                "no WATERMARK_PJM_API_KEY set — a PJM Data Miner 2 subscription key is required for "
