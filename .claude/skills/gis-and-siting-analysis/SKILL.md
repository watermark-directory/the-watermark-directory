...
The project layer supplies the actual datasets, endpoints, parcel identifiers, scoring profiles, and the corridor(s) under study. In *Project BOSC* the provenanced-value convention is the `ProvenancedValue` model in `watermark.hydrology.model` (a value carries `source` / `citation` / `confidence` / `asof`); spatial layers live in `watermark.gis` (`sites`, `corridor`, `raster`, `imagery`, `analysis`) over committed GeoJSON rather than live ArcGIS REST. Bind to those; keep this file endpoint-free. See `docs/investigative-method/ENRICHMENT.md`.
